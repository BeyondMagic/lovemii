...
function gradient ({
	...
	return {
		red: 143 / 255 - 18 / 255 * range,
		green: 20 / 255 + 185 / 255 * range,
		blue: 50 / 255 + 75 / 255 * range,
	}
}
...
function draw_icon (label : ReturnType<typeof Label>, context : any) : void {
	...
	const color = gradient({ percentage : Battery.percent })
...
function colour ({
	...
	const color = gradient({ percentage })

	const number = '<span foreground="#'
		+ to_hex({ color: color.red })
		+ to_hex({ color: color.green })
		+ to_hex({ color: color.blue } )
		+ '">'
		+ percentage
		+ markup_percentage
		+ '</span>'
